...
=> 0x1000:	auipc	t0,0x0 
   # 将0x0的高12位与PC的高20位相加，结果存入寄存器t0，即t0=0+pc=0x1000
   0x1004:	addi	a1,t0,32 # a1=t0+32=0x1020 
   0x1008:	csrr	a0,mhartid 
   # 从控制和状态寄存器（CSR）中读取数据，将当前硬件线程的ID存入a0寄存器中。对于多线程或多核处理器的编程非常重要，能够帮助识别当前正在运行的线程。
...
(gdb) x/10i $pc
=> 0x80000000:	csrr	a6,mhartid  # 从控制和状态寄存器中读取当前硬件线程ID到寄存器a。
   0x80000004:	bgtz	a6,0x80000108  # 如果a6大于零则跳转到地址0x80000108。如果a6小于或等于零，继续执行下一条指令。
   ...
   0x80000014:	addi	t1,t1,-16  # t1=0x80000010-16=0x8000000A
   0x80000018:	sd	t1,0(t0)  # 将t1中的值（0x8000000A）存储到内存地址0x80000408中
...
- `jr t0`：据寄存器t0中的地址跳转执行,即跳转到地址0x80000000处，执行OpenSBI。
...
### Lab 0.5
#### 1.makefile的作用
- 自动完成代码编译和链接，不需要手动输入一大堆命令。
- 如果修改了某个源文件，Makefile 能自动重新编译受影响的部分，确保最终的程序是最新的。
- 可以在makefile中定义一些常用的命令和选项，之后只需输入简单的命令来运行它们。
- 可以在不同的计算机和环境中使用，帮助你在不同的平台上保持代码一致性

#### 2.最小可执行内核概述
在构建最小可执行内核时，我们主要需要完成两个任务：
1. **内核的内存布局和入口点设置**：内核必须在特定的物理内存地址加载，QEMU 会将 OpenSBI 固件加载到物理地址 `0x80000000`，而内核镜像（`os.bin`）则加载到 `0x80200000`。
2. **通过 OpenSBI 封装输入输出函数**：OpenSBI 作为 bootloader，会负责加载操作系统，并将 CPU 的程序计数器（PC）跳转到内核的入口点，开始执行内核。

OpenSBI 如何确定加载位置
- **地址选择**：OpenSBI 在加载操作系统时必须确保第一条指令位于物理地址 `0x80200000`，这是由 QEMU 规定的。
- **可执行文件格式**：我们使用 ELF 和 BIN 两种格式：
    - **ELF 文件**：格式复杂，包含文件头、调试信息和段信息。虽然可以解析，但对于 OpenSBI 来说，处理起来较为繁琐。
    - **BIN 文件**：简单，文件头之后直接指定加载位置。由于其直接性，BIN 文件更适合用于 OpenSBI。

**任务总结**
- 生成合适内存布局的 ELF 文件，并使用 `objcopy` 将其转换为 BIN 文件。
- 加载 BIN 文件到 QEMU 中运行，OpenSBI 将完成加载和跳转到内核的工作。
- 跳转到内核后，内核首先会初始化基本环境，包括设置数据结构、内存管理、和中断处理机制。接着，内核会加载和初始化设备驱动程序，建立调度系统以合理分配 CPU 时间。随后，内核启动第一个用户空间进程（通常是 `init` 进程），配置其环境并最终进入用户态，开始执行用户程序，从而完成系统的启动和运行。

### Lab 1
#### 1. 上下文环境的保存与恢复
   中断或者异常发生时，需要进行上下文切换——对应操作系统的进程切换：
   - 当操作系统终止当前进程时，将通用寄存器的值保存到内存或堆栈中->保存特殊寄存器的值（CSR）->保存pc、sp等；
   - 根据中断或异常类型执行相应的函数，进行相应的操作；
   - 恢复寄存器状态->恢复pc、sp等->恢复csr等，根据需要，有些csr不用恢复。
#### 2. 时钟中断的处理
- 中断请求被处理器接收后，处理器会根据中断向量表（Interrupt Vector Table）或中断描述符来查找与时钟中断相关联的中断处理程序->保存任务的上下文环境->时钟中断处理->恢复上下文
-  时钟中断处理程序执行的与时钟中断相关的操作：
    - 更新任务的时间片（如果采用时间片轮转调度）；
    - 执行任务切换，选择下一个要运行的任务；s
    - 更新系统时间，维护系统时钟；
    - 处理与时间相关的任务，例如定时器事件等。
